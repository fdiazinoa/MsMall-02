...
export const ApiService = {
  ...
  async getImportConfigs(mallId?: string): Promise<ImportConfig[]> {
    ...
    return (data || []).map((local: any) => ({
      ...
      puerto: local.sftp_port || (['API', 'WEBSERVICE'].includes(local.sftp_protocol) ? 443 : 22),
  ...
  async getRemoteConnections(mallId: string, token?: string): Promise<RemoteConnection[]> {
    ...
    return (data || []).map((row: any) => ({
      ...
      host: row.host || '',
      puerto: Number(row.puerto || (['API', 'WEBSERVICE'].includes(row.protocolo) ? 443 : 22)),
      usuario: row.usuario || '',
  ...
  async saveRemoteConnection(
    ...
    return {
      ...
      host: data.host || '',
      puerto: Number(data.puerto || (['API', 'WEBSERVICE'].includes(data.protocolo) ? 443 : 22)),
  ...
  async testConnection(config: Partial<ImportConfig>, password?: string, token?: string): Promise<{ success: boolean, message: string }> {
    const requestBody = JSON.stringify({
      ...
      puerto: Number(config.puerto) || (config.protocolo === 'SFTP' ? 22 : ['API', 'WEBSERVICE'].includes(config.protocolo) ? 443 : 21),
  ...
  async getLoadLogs(mallId?: string, token?: string, limit: number = 50): Promise<LoadLogEntry[]> {
    try {
      const query = new URLSearchParams();
      if (mallId) query.set('mall_id', mallId);
      query.set('limit', String(Math.max(1, Math.min(Math.trunc(limit), 1000))));
