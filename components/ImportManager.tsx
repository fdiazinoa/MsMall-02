...
const getWebserviceRangeValidationError = (config: Partial<ImportConfig>): string | null => {
  if (!isSubaWebserviceConfig(config) && !isMalalaWebserviceConfig(config)) return null;
...
const withWebserviceDefaults = (config: ImportConfig): ImportConfig => {
  if (!isSubaWebserviceConfig(config) && !isMalalaWebserviceConfig(config)) return config;

  const malalaDefaults = isMalalaWebserviceConfig(config)
    ? {
...
export const ImportManager: React.FC<ImportManagerProps> = ({ initialSection = 'ftp', onCloseWebserviceModal }) => {
  ...
  const handleSave = async () => {
    ...
    const missing = editingConfig.protocolo === 'API'
      ? []
      : STANDARD_FIELDS.filter(f => f.required && !hasFieldMappingValue(editingConfig, f.key));
    if (missing.length > 0) {
    ...
    if (editingConfig.protocolo === 'API') {
      ...
      if (!existingApiConfig && !tempPassword.trim() && !editingConfig.password?.trim()) {
        alert(
          selectedApiProvider === 'cookieontop' ? 'Indica el token Bearer de CookieOnTop.' : selectedApiProvider === 'invupos'
    ...
    if (editingConfig.protocolo === 'WEBSERVICE') {
      ...
      const rangeError = getWebserviceRangeValidationError(editingConfig);
      if (rangeError) {
        alert(rangeError);
        return;
      }
    ...
    const configToSave = {
      ...withWebserviceDefaults(editingConfig),
      password: tempPassword || editingConfig.password
    };
    try {
  ...
  const hasRequiredMapping = (config: ImportConfig) => {
    if (config.protocolo === 'API') return true;
                    ...
                    <div className="flex-1">
                      ...
                      <select
                        ...
                        onChange={e => {
                          ...
                          const constants = { ...(editingConfig.constants || {}) };
                          if (nextProtocol === 'API') {
                            constants.provider = 'studio_g';
                            constants[studioGDateModeKey] = constants[studioGDateModeKey] || 'yesterday';
                          }
                          setEditingConfig({
                            ...editingConfig,
                          ...
                          <select
                            ...
                            onChange={e => {
                              ...
                              setEditingConfig({
                                ...editingConfig,
                                constants,
                                host: provider === 'cookieontop' ? cookieontopEndpoint : provider === 'invupos'
                                  ? invuposEndpoint
                                  : provider === 'bundaberg'
                                    ? bundabergEndpoint
                                    : ([bundabergEndpoint, invuposEndpoint, cookieontopEndpoint].includes(editingConfig.host) ? 'https://alcagora.ddns.net' : editingConfig.host),
                                usuario: provider !== 'studio_g' ? '' : editingConfig.usuario,
                                password: '',
                                ruta_remota: provider === 'cookieontop' ? '' : provider === 'invupos'
                                  ? 'citas/viewAll'
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            placeholder={editingConfig.protocolo === 'WEBSERVICE'
                              ? malalaSalesEndpoint
                              : editingConfig.protocolo === 'API'
                                ? (selectedApiProvider === 'cookieontop' ? cookieontopEndpoint : selectedApiProvider === 'invupos' ? invuposEndpoint : selectedApiProvider === 'bundaberg' ? bundabergEndpoint : 'https://alcagora.ddns.net')
                              : 'sftp.tu-tienda.com'}
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">
                        {editingConfig.protocolo === 'WEBSERVICE'
                          ...
                          : editingConfig.protocolo === 'API'
                          ? (selectedApiProvider === 'cookieontop' ? 'Autenticación Bearer token' : ['bundaberg', 'invupos'].includes(selectedApiProvider) ? 'Autenticación API key' : 'Autenticación Client Credentials')
                          ...
                          <input
                            ...
                            placeholder={hasStoredCredential && !tempPassword
                              ...
                              : editingConfig.protocolo === 'WEBSERVICE'
                                ...
                                : editingConfig.protocolo === 'API'
                                ? (selectedApiProvider === 'cookieontop' ? 'Token Bearer de CookieOnTop' : selectedApiProvider === 'invupos' ? 'APIKEY de InvuPOS' : selectedApiProvider === 'bundaberg' ? 'API key de Bundaberg' : 'Client Secret')
                    ...
                    <>
                      <div>
                        ...
                        <input
                          type="text"
                          className="w-full px-4 py-2.5 rounded-xl border border-slate-200 outline-none"
                          placeholder={selectedApiProvider === 'cookieontop' ? 'Vacío: todas las ventas autorizadas por el token' : selectedApiProvider === 'invupos' ? 'citas/viewAll' : selectedApiProvider === 'bundaberg' ? '8906' : 'AFB'}
