...
import { OperationalFinding } from '../types';
...
export const OperationsCenter: React.FC<{ onOpenBigData?: () => void }> = ({ onOpenBigData }) => {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const response = await ApiService.getOperationsFindings(mallId, token, {
  ...
  }, [currentMall?.id, token, filters.severity, filters.status, filters.type]);
  ...
  return <div className="space-y-4 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className={`rounded-full border px-2 py-0.5 text-[11px] font-bold ${severityTone[finding.severity] || severityTone.INFO}`}>{finding.severity}</span>
            <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-600">{finding.type}</span>
          ...
          <h3 className="mt-2 font-bold text-slate-800">{finding.title}</h3>
          <p className="mt-1 text-sm text-slate-600">{finding.description}</p>
          <p className="mt-2 text-xs text-slate-400">{formatDate(finding.detected_at)} · {finding.source}</p>
        </div>
        <div className="flex shrink-0 flex-wrap gap-2">
          ...
          {finding.status === 'OPEN' && <button disabled={working === finding.id} onClick={() => applyAction(finding, 'review')} className="rounded-lg border border-indigo-200 px-2.5 py-1.5 text-xs font-bold text-indigo-600">Revisado</button>}
          {finding.status !== 'RESOLVED' ? <button disabled={working === finding.id} onClick={() => applyAction(finding, 'resolve')} className="inline-flex items-center gap-1 rounded-lg bg-emerald-600 px-2.5 py-1.5 text-xs font-bold text-white"><CheckCircle2 size={13} /> Resolver</button>
    ...
    {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
      <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wider text-indigo-500">Detalle de hallazgo</p><h3 className="mt-1 text-xl font-bold text-slate-800">{selected.title}</h3></div><button onClick={() => setSelected(null)} className="text-sm text-slate-500">Cerrar</button></div>
