...
import { OperationalFinding, OperationsFindingsResponse } from '../types';
...
export const OperationsCenter: React.FC = () => {
  ...
  const loadFindings = async () => {
    ...
    setLoading(true);
    try {
      const response = await ApiService.getOperationalFindings(currentMall.id, token, { status: 'OPEN', limit: 150 });
      setData(response);
    } catch (error: any) {
