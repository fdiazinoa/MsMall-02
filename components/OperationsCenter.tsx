  ...
  <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
      <div className="min-w-0">
        ...
        <h3 className="mt-2 text-base font-black text-slate-900">{businessTitle(finding)}</h3>
        <p className="mt-1 text-sm leading-5 text-slate-600">{businessReason(finding)}</p>
      </div>
...
export const OperationsCenter: React.FC = () => {
  ...
  return (
    <div className="space-y-3 lg:h-[calc(100dvh-9rem)] xl:h-[calc(100dvh-8rem)] lg:min-h-[520px] xl:min-h-[580px] lg:overflow-y-auto lg:pr-1">
        ...
        <div className="relative px-4 py-3">
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-xl font-black tracking-tight">Estado Operativo del Mall</h2>
            ...
            <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-black text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50"
              >
      ...
      <section className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm xl:col-span-1">
          ...
          <h3 className="mt-1 text-lg font-black text-slate-900">Qué cambió y qué atender</h3>
          <p className="mt-3 text-sm leading-5 text-slate-700">{operationalSummary}</p>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm xl:col-span-2">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h3 className="mt-1 text-lg font-black text-slate-900">Locales que requieren atención</h3>
            </div>
            <button
          ...
          <div className="mt-3 max-h-[260px] space-y-2.5 overflow-y-auto pr-1">
            {model.priorityLocations.length === 0 ? (
              ...
              <div key={`${item.local_id || item.local_name}-${index}`} className="rounded-xl border border-slate-100 bg-slate-50 p-3">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h3 className="mt-1 text-lg font-black text-slate-900">Ordenado por mayor riesgo operativo</h3>
            ...
            <tbody className="divide-y divide-slate-100">
              {model.locations.slice(0, 12).map((local, index) => (
                <tr key={`${local.local_id || local.local_name}-${index}`} className="align-top">
                  <td className="px-3 py-2.5 font-black text-slate-900">{local.local_name}</td>
                  <td className="px-3 py-2.5">
                    <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-black ${scoreTone(local.score)}`}>{local.score}</span>
                  </td>
                  <td className="px-3 py-2.5 text-slate-700">{local.status}</td>
                  <td className="px-3 py-2.5 text-slate-500">{formatDateTime(local.last_activity)}</td>
                  <td className="px-3 py-2.5 text-slate-700">{local.missing_days || 0}</td>
                  <td className="px-3 py-2.5 text-slate-700">{local.action || 'Continuar monitoreo operativo.'}</td>
                </tr>
              ))}
            </tbody>
          </table>
