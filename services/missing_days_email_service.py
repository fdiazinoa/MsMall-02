...
def build_consolidated_missing_days_email_html(
    *,
    ...
    """Build one administrative summary for all audited locals in a mall."""
    safe_mall = escape(mall_name or "Mall")
    safe_start = escape(fecha_inicio)
    safe_end = escape(fecha_fin)
...
def send_consolidated_missing_days_email_for_mall(
    ...
    *,
    ...
    admin_emails = [
        email
        for email in dict.fromkeys(admin_emails)
    ...
    if not summaries_for_email:
        return {
            ...
            "results": [{
                ...
                "status": "skipped",
                "missing_days": 0,
                "reason": "No hay locales con dias faltantes en el periodo.",
            }],
        }
    ...
    context = missing_days_template_context(
        mall_name=mall_name,
        local_name=f"{len(summaries_for_email)} locales",
        fecha_inicio=fecha_inicio,
        fecha_fin=fecha_fin,
    ...
    html_body = build_consolidated_missing_days_email_html(
        ...
        fecha_inicio=fecha_inicio,
        fecha_fin=fecha_fin,
        local_summaries=summaries_for_email,
        report_url=report_url,
    ...
    for settings in rows:
        ...
        if not claimed:
            runs.append({
                ...
                "notification_type": notification_type,
                "executed": False,
                "reason": "already_claimed_for_slot",
                "slot": slot,
            })
            continue
