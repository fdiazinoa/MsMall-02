-- Preserve the legacy store-name contract exactly.
-- Business type and rubro labels remain trimmed, but store names keep their
-- configured whitespace until a separate data-cleanup change is approved.
...
with allowed_locales as (
  select
    l.id,
    coalesce(nullif(l.nombre, ''), 'Local sin nombre') as local_name,
